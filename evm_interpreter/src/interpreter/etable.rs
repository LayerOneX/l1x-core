	...
	ES: EtableSet<S, H, Tr>,
{
	...
	pub fn new(machine: Machine<S>, etable: &'etable ES) -> Self {
		...
		Self { machine, valids, position: 0, etable, _marker: PhantomData }
	}
	...
	pub fn inspect(&self) -> Option<(Opcode, &Stack)> {
		self.code.get(self.position).map(|v| (Opcode(*v), &self.stack))
	}
	...
	ES: EtableSet<S, H, Tr>,
{
	...
	fn step(&mut self, handle: &mut H) -> Result<(), Capture<ExitResult, Tr>> {
		...
		let control = self.etable.eval(&mut self.machine, handle, opcode, self.position);
