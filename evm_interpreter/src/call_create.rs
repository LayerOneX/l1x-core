...
impl CallCreateTrapData {
	...
	pub fn new_from<S: AsRef<RuntimeState> + AsMut<RuntimeState>>(
		...
		match opcode {
			...
			Opcode::CALL => Ok(Self::Call(CallTrapData::new_from(CallScheme::Call, machine)?)),
			Opcode::CALLCODE =>
				Ok(Self::Call(CallTrapData::new_from(CallScheme::CallCode, machine)?)),
			Opcode::DELEGATECALL =>
				Ok(Self::Call(CallTrapData::new_from(CallScheme::DelegateCall, machine)?)),
			Opcode::STATICCALL =>
				Ok(Self::Call(CallTrapData::new_from(CallScheme::StaticCall, machine)?)),
...
impl CallTrapData {
	...
	fn new_from_params<S: AsRef<RuntimeState> + AsMut<RuntimeState>>(
		...
		let in_end = in_offset.checked_add(in_len).ok_or(ExitException::InvalidRange)?;
		let out_end = out_offset.checked_add(out_len).ok_or(ExitException::InvalidRange)?;
		...
		let transfer = if scheme == CallScheme::Call {
			Some(Transfer { source: state.as_ref().context.address, target: (*to).into(), value })
	...
	pub fn feedback<S: AsRef<RuntimeState> + AsMut<RuntimeState>>(
		...
		let ret = match reason {
			Ok(_) => {
				match machine.memory.copy_large(out_offset, U256::zero(), target_len, &retbuf[..]) {
					Ok(()) => {
			...
			Err(ExitError::Reverted) => {
				...
				let _ =
					machine.memory.copy_large(out_offset, U256::zero(), target_len, &retbuf[..]);
	...
	pub fn has_value(&self) -> bool {
		self.transfer.as_ref().map(|t| t.value != U256::zero()).unwrap_or(false)
	}
}
...
impl CreateTrapData {
	pub fn new_create_from<S: AsRef<RuntimeState> + AsMut<RuntimeState>>(
		...
		stack.perform_pop3_push0(|value, code_offset, code_len| {
			...
			let scheme = CreateScheme::Legacy { caller: state.as_ref().context.address };

			state.as_mut().retbuf = Vec::new();

			Ok(((), Self { scheme, value, code }))
		})
	}
