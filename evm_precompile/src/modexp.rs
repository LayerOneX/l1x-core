...
use core::cmp::max;
use evm_interpreter::{ExitException, ExitResult, ExitSucceed, GasMutState};
...
impl<G: GasMutState> PurePrecompile<G> for Modexp {
	fn execute(&self, input: &[u8], gasometer: &mut G) -> (ExitResult, Vec<u8>) {
		...
		if base_len_big > max_size_big {
			try_some!(Err(ExitException::Other("unreasonably large base length".into())));
		}

		let exp_len_big = BigUint::from_bytes_be(&exp_len_buf);
		if exp_len_big > max_size_big {
			try_some!(Err(ExitException::Other("unreasonably large exponent length".into())));
		}

		let mod_len_big = BigUint::from_bytes_be(&mod_len_buf);
		if mod_len_big > max_size_big {
			try_some!(Err(ExitException::Other("unreasonably large modulus length".into())));
		}
