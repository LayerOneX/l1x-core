...
use crate::{
	config::Config, invoker::InvokerState, Control, ExitError, ExitException, GasMutState,
	GasState, Machine, MergeStrategy, Opcode, RuntimeBackend, RuntimeState, Stack,
};
use alloc::vec::Vec;
...
impl<'config> GasometerState<'config> {
	...
	/// Refunds are capped to prevent abuse. Specifically, 
	/// the amount of gas refunded cannot exceed half 
	/// of the gas used by the transaction.
	/// This is to avoid scenarios where transactions could execute 
	/// with little to no cost by manipulating state changes that are refunded.
	/// https://github.com/ethereum/go-ethereum/blob/ad4fb2c7291972a1940dbb276ed1b6f49906767c/core/state_transition.go#L471
	/// Calculates the effective burnt gas, accounting for the refund limit.
	pub fn effective_burnt_gas(&self) -> u64 {
		let refunded = std::cmp::min(self.refunded_gas, self.used_gas / 2);
		self.used_gas - refunded
	}

    /// Returns the amount of gas that has been refunded.
    pub fn get_refunded_gas(&self) -> u64 {
        self.refunded_gas
    }
	...
	pub fn new(gas_limit: u64, is_static: bool, config: &'config Config) -> Self {
		Self { gas_limit, memory_gas: 0, used_gas: 0, refunded_gas: 0, is_static, config }
	}
	...
	H: RuntimeBackend,
{
	...
	let address = AsRef::<RuntimeState>::as_ref(&machine.state).context.address;
...
fn dynamic_opcode_cost<H: RuntimeBackend>(
	...
	let gas_cost = match opcode {
		...
		Opcode::EXTCODESIZE => {
			...
			GasCost::ExtCodeSize { target_is_cold: handler.is_cold(target, None) }
		},
		Opcode::BALANCE => {
			let target = stack.peek(0)?.into();
			GasCost::Balance { target_is_cold: handler.is_cold(target, None) }
		},
		Opcode::BLOCKHASH => GasCost::BlockHash,

		Opcode::EXTCODEHASH if config.has_ext_code_hash => {
			let target = stack.peek(0)?.into();
			GasCost::ExtCodeHash { target_is_cold: handler.is_cold(target, None) }
		},
		...
		Opcode::SHA3 => GasCost::Sha3 { len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::EXTCODECOPY => {
			let target = stack.peek(0)?.into();
			GasCost::ExtCodeCopy {
				target_is_cold: handler.is_cold(target, None),
		...
		Opcode::CALLDATACOPY | Opcode::CODECOPY =>
			GasCost::VeryLowCopy { len: U256::from_big_endian(&stack.peek(2)?[..]) },
		Opcode::EXP => GasCost::Exp { power: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::SLOAD => {
			let index = stack.peek(0)?;
			GasCost::SLoad { target_is_cold: handler.is_cold(address, Some(index)) }
		},
		...
		Opcode::RETURNDATACOPY if config.has_return_data =>
			GasCost::VeryLowCopy { len: U256::from_big_endian(&stack.peek(2)?[..]) },
		...
		Opcode::LOG0 if !is_static =>
			GasCost::Log { n: 0, len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::LOG1 if !is_static =>
			GasCost::Log { n: 1, len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::LOG2 if !is_static =>
			GasCost::Log { n: 2, len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::LOG3 if !is_static =>
			GasCost::Log { n: 3, len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::LOG4 if !is_static =>
			GasCost::Log { n: 4, len: U256::from_big_endian(&stack.peek(1)?[..]) },
		Opcode::CREATE if !is_static => GasCost::Create,
		Opcode::CREATE2 if !is_static && config.has_create2 =>
			GasCost::Create2 { len: U256::from_big_endian(&stack.peek(2)?[..]) },
...
impl GasCost {
	...
	pub fn cost(&self, gas: u64, config: &Config) -> Result<u64, ExitError> {
		Ok(match *self {
			GasCost::Call { value, target_is_cold, target_exists, .. } =>
				costs::call_cost(value, target_is_cold, true, true, !target_exists, config),
			GasCost::CallCode { value, target_is_cold, target_exists, .. } =>
				costs::call_cost(value, target_is_cold, true, false, !target_exists, config),
			GasCost::DelegateCall { target_is_cold, target_exists, .. } =>
				costs::call_cost(U256::zero(), target_is_cold, false, false, !target_exists, config),
			GasCost::StaticCall { target_is_cold, target_exists, .. } =>
				costs::call_cost(U256::zero(), target_is_cold, false, true, !target_exists, config),

			GasCost::Suicide { value, target_is_cold, target_exists, .. } =>
				costs::suicide_cost(value, target_is_cold, target_exists, config),
			GasCost::SStore { original, current, new, target_is_cold } =>
			...
			GasCost::ExtCodeCopy { target_is_cold, len } =>
	...
	pub fn refund(&self, config: &Config) -> i64 {
		match *self {
			...
			GasCost::Suicide { already_removed, .. } if !config.decrease_clears_refund =>
...
mod tests {
	...
	#[test]
	fn test_transaction_cost_create_no_data_no_access_list() {
		let data = vec![];
		let access_list = vec![];

		let transaction_cost = TransactionCost::create(&data, &access_list);
		...
		match transaction_cost {
			TransactionCost::Create {
				...
				assert_eq!(access_list_address_len, 0);
				assert_eq!(access_list_storage_len, 0);
				assert_eq!(initcode_cost, 0);
			},
			_ => panic!("Expected TransactionCost::Create"),
		}
	}

	#[test]
	fn test_transaction_cost_create_non_zero_data() {
		let data = vec![1u8; 64];
		let access_list = vec![];
		...
		match transaction_cost {
			TransactionCost::Create {
				...
				assert_eq!(zero_data_len, 0);
				assert_eq!(non_zero_data_len, 64);
				assert_eq!(access_list_address_len, 0);
				assert_eq!(access_list_storage_len, 0);
				assert_eq!(initcode_cost, 4);
			},
			_ => panic!("Expected TransactionCost::Create"),
		}
	}
	...
	#[test]
	fn test_transaction_cost_create_large_access_list() {
		let data = vec![0u8; 64];
		let access_list = vec![
			(H160::default(), vec![H256::default(); 10]),
			(H160::default(), vec![H256::default(); 20]),
		];
		...
		match transaction_cost {
			TransactionCost::Create {
				...
				assert_eq!(zero_data_len, 64);
				assert_eq!(non_zero_data_len, 0);
				assert_eq!(access_list_address_len, 2);
				assert_eq!(access_list_storage_len, 30);
				assert_eq!(initcode_cost, 4);
			},
			_ => panic!("Expected TransactionCost::Create"),
		}
