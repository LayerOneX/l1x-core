use super::{consts::*, util::log2floor};
use crate::{config::Config, ExitException};
...
pub fn verylowcopy_cost(len: U256) -> Result<u64, ExitException> {
	...
	let gas = U256::from(G_VERYLOW)
		.checked_add(
			U256::from(G_COPY)
				.checked_mul(if wordr == U256::zero() { wordd } else { wordd + U256::one() })
				.ok_or(ExitException::OutOfGas)?,
		)
		.ok_or(ExitException::OutOfGas)?;
...
pub fn log_cost(n: u8, len: U256) -> Result<u64, ExitException> {
	let gas = U256::from(G_LOG)
		.checked_add(U256::from(G_LOGDATA).checked_mul(len).ok_or(ExitException::OutOfGas)?)
		.ok_or(ExitException::OutOfGas)?
...
pub fn sstore_cost(
	...
	Ok(
		...
		if is_cold { gas_cost + config.gas_sload_cold } else { gas_cost },
	)
}
...
pub fn suicide_cost(value: U256, is_cold: bool, target_exists: bool, config: &Config) -> u64 {
	...
	let should_charge_topup =
		if eip161 { value != U256::zero() && !target_exists } else { !target_exists };

	let suicide_gas_topup = if should_charge_topup { config.gas_suicide_new_account } else { 0 };
