...
impl StakingState {
    pub async fn new() -> Result<Self, Error> {
        ...
        let staking_state = StakingState { session: db_session.clone() };
        staking_state.create_tables().await?;
        Ok(staking_state)
    }

    pub async fn get_state(session: Session) -> Result<Self, Error> {
        let db_session = DatabaseManager::get_session().await?;
        let state = StakingState { session: db_session.clone() };
    ...
    pub async fn get_staking_pool(&self, pool_address: &Address) -> Result<StakingPool, Error> {
        ...
        let pool_owner: Address = if let Some(row) = rows.get(0) {
        ...
        let contract_instance_address: Option<Address> = if let Some(row) = rows.get(0) {
        ...
        let cluster_address: Address = if let Some(row) = rows.get(0) {
        ...
        let created_block_number: BlockNumber = if let Some(row) = rows.get(0) {
        ...
        let updated_block_number: BlockNumber = if let Some(row) = rows.get(0) {
        ...
        let min_stake: Option<Balance> = if let Some(row) = rows.get(0) {
        ...
        let max_stake: Option<Balance> = if let Some(row) = rows.get(0) {
        ...
        let min_pool_balance: Option<Balance> = if let Some(row) = rows.get(0) {
        ...
        let max_pool_balance: Option<Balance> = if let Some(row) = rows.get(0) {
        ...
        let staking_period: Option<Balance> = if let Some(row) = rows.get(0) {
