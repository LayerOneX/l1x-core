...
impl ValidateVoteResult {
	pub async fn validate_vote_result<'a>(vote_result: &VoteResult, block: &Block, db_pool_conn: &'a DbTxConn<'a>) -> Result<(), Error> {
		...
		debug!("🛡️  Validate Vote Result - Received Block number: {}, Vote result block number: {}", block.block_header.block_number, vote_result.data.block_number);
		debug!("🛡️  Validate Vote Result - Received Block hash: {}, Vote result block hash: {}", hex::encode(block.block_header.block_hash), hex::encode(vote_result.data.block_hash));
		debug!("🛡️  Validate Vote Result - Received Cluster address: {}, Vote result cluster address: {}", hex::encode(block.block_header.cluster_address), hex::encode(vote_result.data.cluster_address));
		if block.block_header.block_number != vote_result.data.block_number {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect block number: actual {}, expected: {}",
		...
		if block.block_header.block_hash != vote_result.data.block_hash {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect block hash: actual {}, expected: {}, block #{}",
		...
		if block.block_header.cluster_address != vote_result.data.cluster_address {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect cluster address: actual {}, expected: {}, block #{}",
		...
		for vote in &vote_result.data.votes {
			...
			match ValidateVoteResult::validate_single_vote(vote, vote_result, block, db_pool_conn).await {
				...
				Err(e) => {
					...
					log::warn!("🛡️ ⚠️  Validate Vote Result - Invalid vote from validator {:?}: {}", 
		...
		log::info!("🛡️ Validate Vote Result - Valid votes: {}, Total votes: {}, Min required votes: {}", valid_votes, total_votes, min_required_votes);
		// Check if we have enough valid votes to consider the vote result valid
		if valid_votes < min_required_votes {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - VoteResult: Insufficient valid votes. Valid: {}, Total: {}, Block #{}",
				valid_votes,
				total_votes,
				block_number
			));
	...
	async fn validate_single_vote<'a>(
		...
		if vote.data.block_number != vote_result.data.block_number {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect Vote block number: actual {}, expected: {}",
		...
		if vote.data.block_hash != vote_result.data.block_hash {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect Vote block hash: actual {}, expected: {}",
		...
		if vote.data.cluster_address != vote_result.data.cluster_address {
			return Err(anyhow!(
				"🛡️ ⚠️  Validate Vote Result - Incorrect Vote cluster address: actual {}, expected: {}",
