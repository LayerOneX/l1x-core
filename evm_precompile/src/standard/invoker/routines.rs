...
use crate::{
	...
	standard::{helper::MergeStrategy, invoker::invoker::InvokerControl},
};
use alloc::vec::Vec;
use evm_interpreter::{
	ExitError, ExitException, ExitResult, Opcode, RuntimeBackend, RuntimeEnvironment, RuntimeState,
	Transfer,
};
...
pub fn enter_call_substack<S, H, R, Tr>(
	...
	state: S,
	handler: &mut H,
) -> Result<(SubstackInvoke, InvokerControl<R::Interpreter, (ExitResult, (S, Vec<u8>))>), ExitError>
where
	S: AsRef<RuntimeState>,
	H: RuntimeEnvironment + RuntimeBackend + TransactionalBackend,
	R: Resolver<S, H, Tr>,
