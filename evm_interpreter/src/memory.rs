...
impl Memory {
	...
	pub fn new(limit: usize) -> Self {
		Self { data: Vec::new(), effective_len: U256::zero(), limit }
	}
	...
	pub fn resize_to_range(&mut self, return_range: Range<U256>) {
		let ret = if return_range.start > U256::from(usize::MAX) {
			...
			let mut ret =
				self.get(return_range.start.as_usize(), usize::MAX - return_range.start.as_usize());
	...
	pub fn set(
		...
		if offset.checked_add(target_size).map(|pos| pos > self.limit).unwrap_or(true) {
...
mod tests {
	...
	fn test_next_multiple_of_32() {
		...
		for x in 0..1024 {
			...
			assert_eq!(Some(U256::from(next_multiple)), next_multiple_of_32(x.into()));
		}
