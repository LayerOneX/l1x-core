...
use crate::{
	...
	standard::{
		...
		invoker::invoker::{Invoker as InvokerT, InvokerControl},
	},
};
use alloc::{rc::Rc, vec::Vec};
use core::{cmp::min, convert::Infallible, marker::PhantomData};
use evm_interpreter::{
	call_create::{CallCreateTrapData, CallTrapData, CreateScheme, CreateTrapData},
	Capture, Context, ExitError, ExitException, ExitResult, ExitSucceed, Interpreter, Opcode,
	RuntimeBackend, RuntimeEnvironment, RuntimeState, TransactionContext, Transfer,
};
...
/// * `H`: Backend type.
/// * `R`: Code resolver type, also handle precompiles. Usually [EtableResolver] but can be
///   customized.
...
impl<'config, 'resolver, S, H, R, Tr> Invoker<'config, 'resolver, S, H, R, Tr> {
	...
	pub fn new(config: &'config Config, resolver: &'resolver R) -> Self {
		Self { config, resolver, _marker: PhantomData }
	}
}
	...
	Tr: IntoCallCreateTrap,
{
	...
	fn new_transact(
		...
		(Self::TransactInvoke, InvokerControl<Self::Interpreter, (ExitResult, (S, Vec<u8>))>),
		ExitError,
	> {
		...
		let address = match &args {
			...
			TransactArgs::Create { caller, salt, init_code, .. } => match salt {
				Some(salt) => {
		...
		let transaction_context = TransactionContext { origin: caller, gas_price };
		let transfer = Transfer { source: caller, target: address, value };

		let work = || -> Result<(TransactInvoke, _), ExitError> {
			match args {
				TransactArgs::Call { caller, address, data, gas_limit, access_list, .. } => {
				...
				TransactArgs::Create { caller, init_code, gas_limit, access_list, .. } => {
	...
	fn enter_substack(
		...
		Result<
			(Self::SubstackInvoke, InvokerControl<Self::Interpreter, (ExitResult, (S, Vec<u8>))>),
			ExitError,
		>,
		Self::Interrupt,
	> {
