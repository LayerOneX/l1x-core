...
use anyhow::Error;
use db::db::DbTxConn;
use db_traits::{base::BaseState, cluster::ClusterState as ClusterStateInternal};
...
impl<'a> ClusterState<'a> {
	pub async fn new(db_pool_conn: &'a DbTxConn<'a>) -> Result<Self, Error> {
		let state: StateInternalImpl<'a> = match &db_pool_conn {
			...
			DbTxConn::ROCKSDB(db_path) => {
				let db_path = format!("{}/cluster", db_path);
		...
		let state = ClusterState { state: Arc::new(state) };
	...
	pub async fn store_cluster_address(&self, cluster_address: &Address) -> Result<(), Error> {
		match &*self.state {
			StateInternalImpl::StateRock(s) => s.store_cluster_address(cluster_address).await,
			StateInternalImpl::StatePg(s) => s.store_cluster_address(cluster_address).await,
			StateInternalImpl::StateCas(s) => s.store_cluster_address(cluster_address).await,
		}
	}
	...
	pub async fn store_cluster_addresses(
		...
		match &*self.state {
			StateInternalImpl::StateRock(s) => s.store_cluster_addresses(cluster_addresses).await,
			StateInternalImpl::StatePg(s) => s.store_cluster_addresses(cluster_addresses).await,
			StateInternalImpl::StateCas(s) => s.store_cluster_addresses(cluster_addresses).await,
		}
	}

	pub async fn load_all_cluster_addresses(&self) -> Result<Option<Vec<Address>>, Error> {
		match &*self.state {
			StateInternalImpl::StateRock(s) => s.load_all_cluster_addresses().await,
			StateInternalImpl::StatePg(s) => s.load_all_cluster_addresses().await,
			StateInternalImpl::StateCas(s) => s.load_all_cluster_addresses().await,
		}
	}
}
